...
def dfa_to_rm(dfa_obj, offset=0):
    '''
    ...
    for state in dfa_dict:
        ...
        for event in dfa_dict[original_state][1]:
            ...
            reward_machine.delta_u[state][event] = next_state + offset
            reward_machine.delta_r[state][next_state + offset] = int(dfa_dict[next_state][0])
            if dfa_dict[original_state][0]:
                reward_machine.T.add(state)
                reward_machine.accepting.add(state) 
    reward_machine.u0 = initial_state + offset
    return reward_machine

def decomposed_dfas_to_single_rm(decomposed_dfas):
    '''
    ...
    current_offset = 1
    for idx, dfa_obj in enumerate(decomposed_dfas):
        rm_obj = dfa_to_rm(dfa_obj, offset=current_offset)
        subsuming_rm._add_state(st for st in rm_obj.get_states())
        subsuming_rm.delta_u[auxiliary_state]["to_rm{}".format(idx + 1)] = rm_obj.get_initial_state()
        subsuming_rm.delta_r[auxiliary_state][rm_obj.get_initial_state()] = 0
        subsuming_rm.delta_r.update(rm_obj.delta_r)
        subsuming_rm.delta_u.update(rm_obj.delta_u)
...
def generate_rm_decompositions(monolithic_rm, num_decompositions, num_agents, disregard_list=None, n_queries=25):
    ...
    while len(decomps) < num_decompositions:
        ...
        if candidate in disregard_list:
            ...
            subsuming_rm = decomposed_dfas_to_single_rm(candidate)
            decomped_rms.append(subsuming_rm)
    return decomped_rms
